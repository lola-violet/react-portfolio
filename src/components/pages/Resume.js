// About
import React from 'react';
import 'bootstrap';
import '../../styles/resume.css';
import resume from '../../assets/lolaapplegate_resume.pdf';

export default function About() {
    return (
        <div className='container'>
            <div className='row'>
                <div className='col-12 mb-5' id='aboutMe'>
                    <h1 className='pageHead'>Resume</h1>
                    <br></br>
                    <p className='skillList'>HTML ~ CSS ~ JavaScript ~ JQuery ~ APIs ~ Bootstrap ~ Materialize</p>
                    <p className='skillList'>Node.js ~ OOP ~ Express ~ MySQL ~ Sequelize ~ BCrypt ~ Handlebars ~ MVC ~ MongoDB ~ Mongoose ~ React ~ Heroku</p>
                    <div className='d-flex justify-content-center'>
                        <a href={resume} target="_blank" rel="noreferrer" className='resume-link card-link'>View Full Resume</a>
                    </div>
                </div>
            </div>
        </div>
    );
}